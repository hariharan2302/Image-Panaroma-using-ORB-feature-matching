...
def detect_faces(img: np.ndarray) -> List[List[float]]:
    ...
    # so we want to call the classifier(path, type_of_classifier)
    ...
    # after turing to grey scale, call the predefined cascade classifier with parameters like
    # scale_factor = 1.05(5% reduction)(slower training but high accuracy to 1.5(vice-versa) and minNeighbours = 3 to 6
    face_detect = my_cascade_face.detectMultiScale(my_grey_img, 1.3, 3, minSize=(13, 13))
    for tx,ty,bw,bh in face_detect:
        detection_results.append([float(tx), float(ty), float(bw), float(bh)])
